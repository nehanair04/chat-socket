 ...
 */
ssize_t rio_readn(SSL *ssl, void *usrbuf, size_t n) {
    size_t nleft = n;
    ssize_t nread;
    char *bufp = usrbuf;
 ...
 */
ssize_t rio_writen(SSL *ssl, const void *usrbuf, size_t n) {
    size_t nleft = n;
    ssize_t nwritten;
...
static ssize_t rio_read(rio_t *rp, char *usrbuf, size_t n) {
    ...
    while (rp->rio_cnt <= 0) { /* Refill if buf is empty */
        rp->rio_cnt = SSL_read(rp->rio_ssl, rp->rio_buf, sizeof(rp->rio_buf));
        if (rp->rio_cnt < 0) {
            if (errno != EINTR) {
                return -1; /* errno set by read() */
            }
 ...
 */
void rio_readinitb(rio_t *rp, SSL *ssl) {
    rp->rio_ssl = ssl;
    rp->rio_cnt = 0;
